...
function evolve_model!(out, ps::Ps, pa::Pa, xs::Xs, xa::Xa, Ipeak::Float64, N, Δt::Float64, t::Float64, tstim::Float64, tstim_dur::Float64, tstimcon::Float64, tstimcon_dur::Float64, Noise::Int, nv, Amp::Float64, Freq::Float64)
    ...
    @inbounds for k in 1:N
        ...
        elseif Noise == 2
          if (tstim   <= (k*Δt) <= (tstim + tstim_dur))
              # nv.x = simulate_ouevar, N, 0., Δt, 0., std, τ)
...
function FindMax(vec, time, thres::Float64)
  ...
  for iter in eachindex(varranges)
      if iter == length(varranges)
				for i = startrange+2:length(tempvec)
					if tempvec[i-1] > tempvec[i] && tempvec[i-2] < tempvec[i-1]
						push!(timemax, tempvectime[i-1])
					end
				end
...
function GetThatHistBoy(vec, Period, nsom, binsAm)

	UsePyHist = false

	if !isempty(vec)
		if UsePyHist
	  	(nsom, bins, patches)=plt[:hist](vec, bins = vecbins)
		else
			BinSize = Period / binsAm
			BinEdges = 0:BinSize:Period
			for j = 1:length(vec)
				tempvec = vec[j]
				for i = 2:length(BinEdges)
					if BinEdges[i-1] < tempvec <= BinEdges[i]
						nsom[i-1] += 1
					end
				end
			end
...
function findIntersect(data, th)
  f(x) = x > th;
...
function fit_hist(xvalues, yvalues, freq)
  amp = (maximum(yvalues) - minimum(yvalues))/2;
  model(xvalues, par) = par[1] + par[2] * sin((xvalues .* 2 * pi * freq) + par[3])
  values = curve_fit(model, xvalues, yvalues, [mean(yvalues), amp, 0])
  Mag = abs(values.param[2]);
  return (values.param[1], Mag, values.param[3]);
end
