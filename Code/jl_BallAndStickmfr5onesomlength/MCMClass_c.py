...
def simulatepar(delay, dur, length, std, offset, amp, freq, dt, tau, mu, loc, simdur, init, cel, seed, loop):
    ...
    Mcm.define_biophysics(2, axon, 0, 500, 0.3e-3, .75*0.75, 150, -65, 55, -70, -25., 6., -35., 6., -15., 4.)
    ...
    h.celsius = cel
    h.run()
    filename = './OUTPUT/spiketrain5Hzmfr' + str(length) + 'length' + str(freq) + 'freq' + str(std) + 'std' + str(seed) + 'seed' + str(loop) + 'loop.txt'
...
def realmain(numprocs, initval, freq, mode):
    ...
    init = -65
    val = 0
    lengthAx = numpy.arange(10, 60, 20)
    std = 0.003
    looplength = numpy.arange(1, 11, 1)
    seedstart = 0
    beginseedstart = 0
    sfr = mode-1
    for length in lengthAx:
        for loopi in looplength:
            ...
            q = p.map_async(simulatepar_one_arg, zip(it.repeat(delay), it.repeat(dur), it.repeat(length), it.repeat(std), it.repeat(curr), it.repeat(amp), freq, it.repeat(dt), it.repeat(tau), it.repeat(mu), it.repeat(loc), it.repeat(simdur), it.repeat(init), it.repeat(cel), seedrange, it.repeat(loopi)))
            while not q.ready():
               time.sleep(1)
            seedstart += len(freq)
        val += 1
