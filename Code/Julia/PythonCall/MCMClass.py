...
def define_biophysics(*arg):
    ...
    6th argument: Membrane Capacitance          (in μF/cm^2)
...
def attach_sine_clamp(cell, delay, dur, offset, amp, freq, loc):
...
def attach_noise_clamp(cell, delay, dur, std, mu, tau, offset, dt, loc):
    """Attach a sinusoidal current Clamp to a cell.
    ...
    :param tau: Relaxing time of noise.
    :param offset: DC current.
    :param dt: Time step of system.
    :param loc: Location on the dendrite where the stimulus is placed.
    """
    stim = h.IClampNoise(cell(loc))
    stim.delay = delay
...
def special_run(stim, soma, axon, simdur, dt, init, cel):
    soma_v_vec = h.Vector()
    axon_v_vec = h.Vector()
    i_inj_vec = h.Vector()
    i_cap_vec = h.Vector()
